...
export function useIncomesPage() {
  ...
  const { filters, updateFilters, clearFilters } = usePageFilters('incomes', defaultFilters);

  const { fetchData, data, loading, error } = useAPI('/incomes');
  ...
  const chartData = computed(() => {
    ...
    const values = data.value.data.map(item => item.barcode || 0);
    return {
      labels,
      datasets: [{
        label: 'Штрих-код',
